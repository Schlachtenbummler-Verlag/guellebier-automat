  ...
  const SS = {
    ...
    init(opts = {}){
      ...
      const defaults = {
        ...
        inactivityMs: 6000,            // nach Inaktivität starten
        scene: "bugs",

        // Spawning
        startBurst: 3,                 // wie viele Käfer sofort beim Start
        spawnEveryMs: 900,             // Spawnrate (ms)

        // Käfer Animation
        bugFps: 10,
        bugSize: 120,                  // unitlos, px (über CSS-Var)
        speedMin: 70,
        speedMax: 130,
        bobAmpMin: 6,
        bobAmpMax: 16,
      ...
      document.addEventListener("pointermove", () => {
        ...
        if (now - _lastMove > 800) { _lastMove = now; reset(); }
      }, { passive:true });
    ...
    start(){
      ...
      if (this._opts.minigame){
        ...
        if (this._opts.minigameHud !== false){
          ...
          this._hud.textContent = 'Zerquetscht: 0';
    ...
    _addKill(speedPxPerSec){
      ...
      if (this._hud) this._hud.textContent = `Zerquetscht: ${this._game.kills}`;
    },
