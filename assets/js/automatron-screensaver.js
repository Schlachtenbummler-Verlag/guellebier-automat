/*! Automatron Screensaver – Bugs mit Mini-Game (Zerquetschen)
    ...
    - Finale Einblendung (3s, zentriert): 
      ...
      Punkte = Summe über (Geschwindigkeit_des_geclickten_Käfers / 2), gerundet.
  ...
  const SS = {
    ...
    init(opts = {}){
      ...
      const defaults = {
        ...
        // Mini-Game
        minigame: true,                // Mini-Game aktiv?
        minigameHud: true,             // HUD anzeigen?
...
// ⭐ WICHTIG: Listener wirklich registrieren
this._overlay.addEventListener('pointerdown', fastStopOnEmpty, { capture: true });

// (dein Reset/Throttle-Block folgt hier…)

     // Interaktionen schalten Idle-Timer wieder scharf – aber nur, wenn NICHT aktiv
const reset = () => { if (!this._active) this._armIdle(); };

// sanft gedrosselt – reagiert auch auf Touch/Scroll/Tasten
const throttle = (fn, ms = 600) => {
  let t = 0;
  return () => { const n = performance.now(); if (n - t > ms) { t = n; fn(); } };
};

// mehr Events, damit es auf Mobilgeräten triggert
['pointermove','pointerdown','pointerup','touchstart','touchmove','touchend','keydown','wheel','scroll']
  .forEach(ev => document.addEventListener(ev, throttle(reset, 600), { passive: true }));
...
window.addEventListener('focus', reset);
    ...
    start(){
      ...
      if (this._opts.minigame){
        ...
        if (this._opts.minigameHud !== false){
          ...
          this._hud.textContent = 'Gepunktete Rübenklemmer zerquetscht: 0';
    ...
    _addKill(speedPxPerSec){
      ...
      // Punkte = Geschwindigkeit/2, gerundet
      const pts = Math.max(1, Math.round((Number(speedPxPerSec)||0) / 10));
      this._game.kills = (this._game.kills|0) + 1;
      this._game.score = (this._game.score|0) + pts;
      if (this._hud) this._hud.textContent = `Gepunktete Rübenklemmer zerquetscht: ${this._game.kills}`;
    },
    ...
    _sceneBugs(scene){
      ...
      const spawnBug = () => {
        ...
        el.addEventListener("pointerdown", (e)=>{
          ...
          // Punkte addieren gemäß Anforderung: speed/2
